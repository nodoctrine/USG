...
The cross-nav HTML is built dynamically by `buildCrossNav()` in `shared-nav.js` from the `COURSES` table. Chapter HTML files contain only an empty placeholder: `<nav class="cross-nav"></nav>`.

To add a chapter or course, update the `COURSES` table in `shared-nav.js` — no chapter HTML edits are needed.
...
5. Cross-nav — leave `<nav class="cross-nav"></nav>` as-is; populated automatically by `shared-nav.js`
...
13. `ACTIVITIES` — one entry per activity block with `tests[]` and `defaultCode`
...
3. **The source material for the chapter** — read from `_Drop Source Content Here/Raw_[CourseName]/` before writing any content
4. **The existing `_template.html`** (read it, substitute placeholders — do not read an existing chapter)

### Source Material Rule

**All chapter content must be derived from the source files in `_Drop Source Content Here/`.** Do not generate content from general knowledge or training data. If no source file exists for a section, stop and ask the user before proceeding.

Source formats by course:
- C Programming: zyBooks HTML exports (`ch01_sec01.html`, `ch02_sec03.html`, etc.)
- Electronics Basics: PDF lecture slide decks (`Chapter 1.pdf`, `Chapter 2.pdf`, etc.)

Read the relevant source files first, identify the key concepts and examples, then write USG content that reflects them. The source is the authority on what topics belong in each chapter and what the correct facts, formulas, and examples are.
